...
class SNTRAVEL_Element_Column extends Element_Column {
     
    protected function content_template() {
        ?>
        <#
        let custom_classes = elementor.hooks.applyFilters('sntravel-custom-column-classes', settings);
        custom_classes = _.isArray(custom_classes)?custom_classes:[];
        var sntravel_column_before = elementor.hooks.applyFilters('sntravel-custom-column/before-render', '', settings, this);
        #>
        <div class="elementor-widget-wrap {{ custom_classes.join(' ') }}">
            {{{sntravel_column_before}}}
    ...
    public function before_render() {
        ...
        $custom_classes = apply_filters('sntravel-custom-column-classes', [], $settings);
        $custom_classes = is_array($custom_classes) ? $custom_classes : [];
        $this->add_render_attribute('_widget_wrapper', 'class', $custom_classes);

        $sntravel_before_column_render = apply_filters('sntravel-custom-column/before-render','', $settings, $this);
        ...
        <div <?php $this->print_render_attribute_string( '_widget_wrapper' ); ?>>
            <?php if(!empty($sntravel_before_column_render)) echo $sntravel_before_column_render; ?>
    ...
    public function after_render() {
        $settings = $this->get_settings_for_display();
        $sntravel_after_column_render = apply_filters('sntravel-custom-column/after-render','', $settings, $this);
        ?>
        <?php if(!empty($sntravel_after_column_render)) echo $sntravel_after_column_render; ?>
            </div>
        </<?php
