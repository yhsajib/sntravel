...
if(!function_exists('sntravel_print_html')){
    function sntravel_print_html($content){
        echo $content;
    }
}
if(!function_exists('sntravel_get_request_data')) {
    function sntravel_get_request_data($filters = array(), $key = ''){
        if ( empty( $filters ) ) {
            return $_REQUEST;
        }else if( !empty($key)){
            return $filters[$key];
...
if(!function_exists('sntravel_print_shortcode')) {
    function sntravel_print_shortcode($content)
    {
        echo do_shortcode($content);
    }
}

if(!function_exists('sntravel_register_shortcode')) {
    function sntravel_register_shortcode($tag, $callback)
    {
        add_shortcode($tag, $callback);
    }
}

if(!function_exists('sntravel_register_wp_widget')) {
    function sntravel_register_wp_widget($class)
    {
        register_widget($class);
    }
}

if(!function_exists('sntravel_remove_theme_filter')) {
    function sntravel_remove_theme_filter($key, $callback)
    {
        remove_filter( $key, $callback, 10, 2 );
    }
}
 
if(!function_exists('sntravel_get_post_categories')){
    function sntravel_get_post_categories(){
...
if(!function_exists('sntravel_get_post_categories_options') ){
    function sntravel_get_post_categories_options(){
        $categories = sntravel_get_post_categories();
        $options = array();
...
if(!function_exists('sntravel_get_grid_term_list')){
    function sntravel_get_grid_term_list($post_type, $taxonomy = array())
    {
        if (empty($taxonomy)) {
...
if(!function_exists('sntravel_get_grid_term_options')){
    function sntravel_get_grid_term_options($post_type, $taxonomy = array())
    {
        if (empty($taxonomy)) {
            $taxonomy = get_object_taxonomies($post_type, 'names');
        }
...
if(!function_exists('sntravel_get_posts_of_grid')) {
    function sntravel_get_posts_of_grid($post_type = 'post', $atts = array(), $taxonomy = array(), $args_extra = array())
    {
        extract($atts);
        if (!empty($post_ids)) {
            $args = array(
            ...
            $sntravel_query->set('paged', intval($sntravel_paged));
            $sntravel_query->set('posts_per_page', !empty($limit) ? intval($limit) : 6);
            $query = $sntravel_query->query($sntravel_query->query_vars);
            $posts = $query;
            ...
            if (empty($source)) {
                $source_new = sntravel_get_grid_term_list($post_type, $taxonomy);
        ...
        $pagination = get_the_posts_pagination(array(
            'screen_reader_text' => '',
            'mid_size' => 2,
            'prev_text' => esc_html__('Back', SNTRAVEL_TEXT_DOMAIN),
            'next_text' => esc_html__('Next', SNTRAVEL_TEXT_DOMAIN),
        ));
        global $paged;
...
if(!function_exists('sntravel_get_term_of_post_to_class')){
    function sntravel_get_term_of_post_to_class($post_id, $tax = array())
    {
        $term_list = array();
        foreach ($tax as $taxo) {
...
if(!function_exists('sntravel_get_all_page')){
    function sntravel_get_all_page(){
...
if(!function_exists('sntravel_attachment_image_src')){
    function sntravel_attachment_image_src( $attachment_id, $images_size, $images_custom_size ) {
...
if(!function_exists('sntravel_share_this_all')){
    function sntravel_share_this_all($args=[]){
        ...
        ob_start();
        ?>
        <span class="sntravel-post-share col-auto">
            <span class="meta-inner">
                <a data-hint="<?php esc_attr_e( 'Share this post to', SNTRAVEL_TEXT_DOMAIN ); ?>"
                    ...
                    <span><?php echo esc_html__( 'Share', SNTRAVEL_TEXT_DOMAIN ) ?></span> </a>
            </span>
        </span>
        <?php  
        $html = ob_get_clean();
        $html = apply_filters( 'sntravel_share_this_all_html', $html );
        if($args['echo']){
            echo $html;
        } else {
            return $html;
...
if(!function_exists('sntravel_post_share_link')){
    function sntravel_post_share_link($args=[]){
        $args = wp_parse_args($args,[
            'icon' => '',
            'post_id'     => get_the_ID(),
            'link_class'  => 'sntravel-icon',
            'icon_class'  => '',
            'icon_svg'  => '',
            'text'        => '',
            'echo'        => true
...
add_action( 'add_meta_boxes_comment', 'sntravel_comment_add_meta_box' );
function sntravel_comment_add_meta_box() {
    add_meta_box( 'comment', __( 'Comment Metadata - Extend Comment' ), 'sntravel_comment_meta_box', 'comment', 'normal', 'high' );
}

function sntravel_comment_meta_box($comment){
    apply_filters( 'sntravel_comment_extra_control', $comment );
}

//add_action( 'restrict_manage_posts', 'sntravel_admin_posts_filter_restrict_manage_posts',10,2 );
function sntravel_admin_posts_filter_restrict_manage_posts($post_type,$which){
    if ($post_type == 'sntravel-template'){
        $template_types = array(
            'header'       => esc_html__('Header', SNTRAVEL_TEXT_DOMAIN), 
            'footer'       => esc_html__('Footer', SNTRAVEL_TEXT_DOMAIN), 
            'mega-menu'    => esc_html__('Mega Menu', SNTRAVEL_TEXT_DOMAIN) 
        );
        $template_types = apply_filters('sntravel_template_type_support',$template_types);
        ?>
        <select name="sntravel_filter_template_type">
            <option value=""><?php esc_html_e('Select Type', SNTRAVEL_TEXT_DOMAIN); ?></option>
            <?php
            $current_v = isset($_GET['sntravel_filter_template_type'])? $_GET['sntravel_filter_template_type']:'';
...
//add_filter( 'parse_query', 'sntravel_posts_filter' );
function sntravel_posts_filter( $query ){
    global $pagenow;
    $type = 'sntravel-template';
    if (isset($_GET['post_type'])) {
        $type = $_GET['post_type'];
    }
    if ( $type == 'sntravel-template' && is_admin() && $pagenow=='edit.php' && isset($_GET['sntravel_filter_template_type']) && $_GET['sntravel_filter_template_type'] != '') {
        $query->query_vars['meta_key'] = 'template_type';
        $query->query_vars['meta_value'] = $_GET['sntravel_filter_template_type'];
    }
}  
...
*/
add_filter( 'update_plugins_7iquid.com', 'sntravel_update_plugins_api', 10, 4 ); //update_plugins_7iquid.tech
function sntravel_update_plugins_api($update, $plugin_data, $plugin_file, $locales){
    ...
    $raw_response = wp_remote_get(
        add_query_arg( ['action' => 'sntravel_get_update_plugins', 'plugin_file' => $plugin_file], $url ),
        array( 'timeout' => 15 )
    );
    if ( $ssl && is_wp_error( $raw_response ) ) {
        $raw_response = wp_remote_get(
            add_query_arg( ['action' => 'sntravel_get_update_plugins', 'plugin_file' => $plugin_file], $http_url ),
            array( 'timeout' => 15 )
        );
    }
