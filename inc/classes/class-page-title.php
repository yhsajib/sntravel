<?php
if (!class_exists('Sntravel_Page_Title')) {
    class Sntravel_Page_Title
    {
        public function get_page_title(){
            $pt_mode = sntravel()->get_opt('pt_mode');
            if( $pt_mode == 'none' ) return;

            $ptitle_layout = (int)sntravel()->get_opt('ptitle_layout');
            $titles = $this->get_title();

            $breadcrumb_on = sntravel()->get_opt( 'breadcrumb_on', false );
            $breadcrumb = new Sntravel_Breadcrumb();
            $entries = $breadcrumb->get_entries();
            $custom_class = sntravel()->get_opt( 'page_title_class', '' );
            $pt_bg_parallax = sntravel()->get_theme_opt('pt_bg_parallax');
            $data_parallax = $pll_cls = '';
            if( (isset($pt_bg_parallax) && $pt_bg_parallax) ){
                $data_parallax = json_encode([
                    sntravel()->get_theme_opt('pt_parallax') => sntravel()->get_theme_opt('pt_parallax_value')
                ]);
            ...
            if ($pt_mode == 'bd' && $ptitle_layout > 0 && class_exists('Pxltheme_Core') && is_callable( 'Elementor\Plugin::instance' )) {
                ?>
                <div id="sntravel-pagetitle" style="<?php if(!empty($bg_image)){?>background-image:url('<?php echo esc_url($bg_image); ?>');<?php } ?>" class="sntravel-pagetitle bg-image layout-el relative <?php echo !empty(esc_attr($custom_class)) ? $custom_class : ''; ?> <?php echo esc_attr($pll_cls) ?>">
                    <div class="sntravel-page-title-bg sntravel-absoluted" data-parallax='<?php echo esc_attr($data_parallax);?>'></div> 
                    <?php echo Elementor\Plugin::$instance->frontend->get_builder_content_for_display( $ptitle_layout);?>
                </div>
                <?php 
            } else {
                ?>
                <div id="sntravel-pagetitle" style="<?php if(!empty($bg_image)){?>background-image:url('<?php echo esc_url($bg_image); ?>');<?php } ?>" class="sntravel-pagetitle bg-image layout-df relative <?php echo esc_attr($custom_class) != '' ? $custom_class : ''; ?> <?php echo esc_attr($pll_cls) ?>">
                    <div class="sntravel-page-title-bg sntravel-absoluted" data-parallax='<?php echo esc_attr($data_parallax);?>'></div>
                    <div class="sntravel-page-title-overlay"></div>
        ...
        public function get_title() {
            $title = '';
            $single_post_title_layout = sntravel()->get_theme_opt('single_post_title_layout','0');
            $post_custom_title  = sntravel()->get_theme_opt('post_custom_title',esc_html__('Blog details', 'sntravel'));
            // Default titles
            if ( ! is_archive() ) {
                // Posts page view
                if ( is_home() ) {
                    ...
                    if ( is_front_page() ) {
                        $title = esc_html__( 'Blog', 'sntravel' );
                    }
                } // Single page view
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404', 'sntravel' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'sntravel' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                ...
                $custom_title = sntravel()->get_opt('custom_title');
            ...
            $main_title = sntravel()->get_page_opt('custom_main_title');
            if (!empty($main_title)){
                $title = $main_title;
            }
            $sub_title = sntravel()->get_opt('custom_sub_title');
            $page_sub_title = sntravel()->get_page_opt('custom_sub_title');
