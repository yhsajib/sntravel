 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class SNTRAVEL_Import extends WP_Importer {
		...
		function process_posts($attachment) {
			...
			foreach ( $this->posts as $post ) {
				...
				if( $post_exists && get_post_type( $post_exists ) == 'product_variation'){
					$post_exists = 0;
				}
				/**
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					if ( 'attachment' == $postdata['post_type'] ) {
						...
						$this->log->putContent('Post: '. $postdata['post_title']. ' has been added', true, true);
					}
		...
		function process_menu_item( $item ) {
			...
			if ( ! $menu_id ) {
				...
				echo '<br />';
				$this->log->putContent('Invalid menu slug '. $menu_slug, true, true);
				return;
			} else {
		...
		function process_attachment( $post, $url ) {
			...
			$this->log->putContent('Media file '. $post['post_title']. ' has been added', true, true);
			//wp_update_attachment_metadata( $post_id, wp_generate_attachment_metadata( $post_id, $upload['file'] ) );
			wp_update_attachment_metadata( $post_id, null );
		 ...
		 */
		function parse( $file ) {
			$parser = new SNTRAVEL_WXR_Parser();
...
/*if( !function_exists('wordpress_importer_init') ) {
	function wordpress_importer_init() {
		...
		$GLOBALS['wp_import'] = new SNTRAVEL_Import();
		register_importer( 'wordpress', 'WordPress', esc_html__('Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', SNTRAVEL_TEXT_DOMAIN), array( $GLOBALS['wp_import'], 'dispatch' ) );
	}
