...
function basilico_woocommerce_remove_product_single_function() {
	...
	function basilico_woocommerce_featured_flash_sale(){
		...
		<div class="hot-sale">
			...
			if ( $product->is_featured() ) {
				...
				?>
				<span class="sntravel-featured" <?php echo 'style="'.esc_html($feature_color_01_style).'"' ?>><?php echo esc_html($feature_text); ?></span>
				...
				if (!empty($feature_text_02)) {
					...
					?>
					<span class="sntravel-featured" <?php echo 'style="'.esc_html($feature_color_02_style).'"' ?>><?php echo esc_html($feature_text_02); ?></span>
					<?php
				}
			}
...
if(!function_exists('basilico_woocommerce_before_single_product_summary')){
	...
	function basilico_woocommerce_before_single_product_summary(){
		$gallery_layout = isset($_GET['gallery_layout']) ? sanitize_text_field($_GET['gallery_layout']) : sntravel()->get_page_opt('gallery_layout', 'simple');
		$thumb_position_cls = ($gallery_layout == 'vertical') ? 'right' : 'bottom';
		$col_cls = ($gallery_layout == 'vertical') ? 'col-lg-6' : 'col-md-6';
		$sidebar_pos = sntravel()->get_theme_opt('product_sidebar_pos', '0');
		...
		if( $gallery_layout == 'vertical' && $sidebar_pos != '0')
			$col_cls = 'col-xl-6';
		$classes = 'sntravel-wc-img-summary sntravel-single-product-gallery-summary-wraps row '.$gallery_layout;
		?>
		<div class="<?php echo esc_attr($classes);?>">
			<div class="sntravel-single-product-gallery-wraps <?php echo esc_attr($col_cls);?> thumbnail-<?php echo esc_attr($thumb_position_cls);?>">
				<div class="sntravel-single-product-gallery-wraps-inner relative">
					<?php
					do_action('basilico_before_single_product_gallery');
					do_action('basilico_single_product_gallery');
					do_action('basilico_after_single_product_gallery');
					?>
				</div>
			</div>
			<div class="sntravel-single-product-summary-wrap <?php echo esc_attr($col_cls);?>">
				<?php
			}
		}

		/* Close summary columns and close gallery-summary wrap */
		if(!function_exists('basilico_woocommerce_after_single_product_summary')){
			add_action('woocommerce_after_single_product_summary', 'basilico_woocommerce_after_single_product_summary', 0);
			function basilico_woocommerce_after_single_product_summary(){
				echo '</div>';
				echo '</div>';
			}
		}
		...
		add_action('basilico_single_product_gallery', 'woocommerce_show_product_images', 1);

		/* Add custom CSS class to Gallery */
		add_filter('woocommerce_single_product_image_gallery_classes','basilico_woocommerce_single_product_image_gallery_classes');
		function basilico_woocommerce_single_product_image_gallery_classes($class){
			$gallery_layout = isset($_GET['gallery_layout']) ? sanitize_text_field($_GET['gallery_layout']) :  sntravel()->get_page_opt('gallery_layout', 'simple');
			$thumb_position_cls = ($gallery_layout == 'vertical') ? 'right' : 'bottom';
			$class[] = 'sntravel-product-gallery-'.$gallery_layout;
			$class[] = 'sntravel-product-gallery-'.$thumb_position_cls;
			unset($class[3]);
			return $class;
		}

		/* Custom thumbnail carousel in bottom Gallery */
		if(!function_exists('basilico_wc_single_product_gallery_layout')){
			add_filter('woocommerce_single_product_carousel_options', 'basilico_wc_single_product_gallery_layout' );
			function basilico_wc_single_product_gallery_layout($options){
				$gallery_layout = isset($_GET['gallery_layout']) ? sanitize_text_field($_GET['gallery_layout']) : sntravel()->get_page_opt('gallery_layout', 'simple');
		...
		if(!function_exists('basilico_product_gallery_thumbnail_sync')){
			add_action('basilico_single_product_gallery', 'basilico_product_gallery_thumbnail_sync', 2);
			function basilico_product_gallery_thumbnail_sync($args=[]){
				global $product;
				$gallery_layout = isset($_GET['gallery_layout']) ? sanitize_text_field($_GET['gallery_layout']) : sntravel()->get_page_opt('gallery_layout', 'simple');
				$thumb_position_cls = ($gallery_layout == 'vertical') ? 'right' : 'bottom';
...
/* Custom Meta */
add_action('woocommerce_single_product_summary','basilico_wc_template_single_meta',32);
if(!function_exists('basilico_wc_template_single_meta')){
	add_action('woocommerce_template_single_excerpt','basilico_wc_template_single_meta',99);
	function basilico_wc_template_single_meta(){
		global $product;
		...
		if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
			$sku = $product->get_sku() ? $product->get_sku() : esc_html__( 'N/A', 'sntravel' );
			echo '<span class="sku_wrapper"><span class="lbl">'.esc_html__( 'SKU:', 'sntravel' ).'</span> <span class="sku">'.$sku.'</span></span>';
		}

		echo wc_get_product_category_list( $product->get_id(), ', ', '<span class="posted_in"><span class="lbl">' . _n( 'Category: ', 'Categories: ', count( $product->get_category_ids() ), 'sntravel' ) . '</span>', '</span>' );

		echo wc_get_product_tag_list( $product->get_id(), ', ', '<span class="tagged_as"><span class="lbl">' . _n( 'Tag: ', 'Tags: ', count( $product->get_tag_ids() ), 'sntravel' ) . '</span>', '</span>' );
...
if(!function_exists('basilico_single_add_to_cart')){
	if (sntravel()->get_theme_opt('product_layout', 'layout-1') == 'layout-4')
...
function basilico_variable_add_to_cart(){
	global $product;
	...
	$btn_style = sntravel()->get_theme_opt('add_to_cart_button_style', 'btn-outline-secondary');
	$single_btn_cls = 'sntravel-btn single_add_to_cart_button button alt '. esc_attr($btn_style);
	?>
	...
	<form class="variations_form cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo esc_attr($variations_attr); ?>">
		...
		<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
			<p class="stock out-of-stock"><?php echo esc_html( apply_filters( 'woocommerce_out_of_stock_message', esc_html__( 'This product is currently out of stock and unavailable.', 'sntravel' ) ) ); ?></p>
		<?php else : ?>
			<div class="sntravel-variation-quantity-wrap">
				<div class="sntravel-variation-wrap">
					<div class="variations">
						<?php foreach ( $attributes as $attribute_name => $options ) : ?>
							<div class="sntravel-variation-row row">
					...
					<div class="sntravel-variation-results">
						<?php
	                ...
	                <div class="sntravel-variations-wrap">
	                	...
	                	<div class="sntravel-addtocart-btn-wrap">
	                		<div class="sntravel-atc-btn">
	    ...
	    function basilico_external_add_to_cart(){
	    	...
	    	$add_to_cart_btn_style = sntravel()->get_theme_opt('add_to_cart_button_style', 'btn-outline-secondary');
	    	$single_btn_cls = 'sntravel-btn single_add_to_cart_button button alt '.esc_attr($add_to_cart_btn_style);
	    	...
	    	$product_wishlist = sntravel()->get_theme_opt('product_wishlist', '0');
	    	if( $product_wishlist == '1') : ?>
	    		<div class="stock-wishlist">
	    			<div class="sntravel-shop-woosmart-wrap">
		...
		if(!function_exists('basilico_woocommerce_template_single_sharing')){
			...
			function basilico_woocommerce_template_single_sharing(){
				$product_social_share_on = sntravel()->get_theme_opt('product_social_share_on', '0');
				if($product_social_share_on != '1') return;
				sntravel()->blog->get_post_share();
			}
		}
		...
		if(!function_exists('basilico_woocommerce_rename_tabs')){
			...
			function basilico_woocommerce_rename_tabs( $tabs ) {
				if(!empty($tabs['additional_information']['title']))
					$tabs['additional_information']['title'] = esc_html__( 'Additional information','sntravel' );	// Rename the additional information tab
		...
		if(!function_exists('basilico_wc_add_to_cart_message_html')){
			...
			function basilico_wc_add_to_cart_message_html($message, $products, $show_qty){
				...
				foreach ( $products as $product_id => $qty ) {
					...
					$titles[] = apply_filters( 'woocommerce_add_to_cart_qty_html', ( $qty > 1 ? absint( $qty ) . ' &times; ' : '' ), $product_id ) . apply_filters( 'woocommerce_add_to_cart_item_name_in_quotes', sprintf( _x( '&ldquo;%s&rdquo;', 'Item name in quotes', 'sntravel' ), strip_tags( get_the_title( $product_id ) ) ), $product_id );
					$count   += $qty;
				}

				$titles = array_filter( $titles );
				/* translators: %s: product name */
				$added_text = sprintf( _n( '%s has been added to your cart.', '%s have been added to your cart.', $count, 'sntravel' ), wc_format_list_of_items( $titles ) );

				// Output success messages.
				if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
					$return_to = apply_filters( 'woocommerce_continue_shopping_redirect', wc_get_raw_referer() ? wp_validate_redirect( wc_get_raw_referer(), false ) : wc_get_page_permalink( 'shop' ) );
					$message   = sprintf( '<span class="sntravel-added-to-cart-msg">%s</span> <a href="%s" tabindex="1">%s</a>', esc_html( $added_text ), esc_url( $return_to ), esc_html__( 'Continue shopping', 'sntravel' ) );
				} else {
					$message = sprintf( '<span class="sntravel-added-to-cart-msg">%s</span> <a href="%s" tabindex="1">%s</a>',esc_html( $added_text ), esc_url( wc_get_cart_url() ), esc_html__( 'View cart', 'sntravel' ) );
				}
				return $message;
			}
		}
		...
		/* Relate Products */
		if (sntravel()->get_theme_opt('product_related', '1') === '0' ){
