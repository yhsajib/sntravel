...
class Pxl_Download_Demo
{
	 ...
	 * @var string
	 */
	private $temp_folder_name = 'sntravelart_temp';
	...
	{
		$sntravel_server_info = apply_filters( 'sntravel_server_info', ['api_url' => 'https://api.7iquid.com/'] ) ;
		$this->api_url = $sntravel_server_info['api_url'] ; 
		add_action( 'wp_ajax_sntravelart_prepare_demo_package', array($this, 'ajax_prepare_demo'), 10, 1 );
		add_action( 'wp_ajax_sntravelart_upload_demo_manual', array($this, 'ajax_upload_demo_manual'), 10, 1 );
	}
	...
	public function download($demo){
		$item = apply_filters( 'sntravel_demo_item_download', get_option( 'template' ) );
