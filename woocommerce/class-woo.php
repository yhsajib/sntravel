...
if (!class_exists('Sntravel_Woo')) {
    class Sntravel_Woo
    {
    	...
    	public function get_single_product_images($product,$args){
			...
			$product_feature_video = sntravel()->get_page_opt('product_feature_video', 'no');
			$video_light_gallery   = isset($_GET['video-lightbox']) ? sanitize_text_field( wp_unslash($_GET['video-lightbox'])) : sntravel()->get_page_opt('video_light_gallery', 'off');  
			$video_output = '';
			if( $product_feature_video != 'no'){
				global $wp_embed;
				$video_url             = sntravel()->get_page_opt('video_url', '');
				$video_file            = sntravel()->get_page_opt('video_file', []);  
				$video_html            = sntravel()->get_page_opt('video_html', '');  
    		...
    		<?php if( $product_layout == 'layout-df'): 
    			...
    			if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'sntravel-product-swiper-slider sntravel-swiper-slider has-thumbs-gallery thumbs-vertical-slider';
    			}

    			$zoom_img_hover = sntravel()->get_theme_opt('zoom_img_hover', 'on');
    			$wrap_cls[] = !empty($video_output) ? 'video-feature' : '';
    			?>
	    		
    			<div class="<?php echo esc_attr( trim(implode(' ', $wrap_cls) )) ?>">
	    			<div class="product-main-img sntravel-light-gallery">
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
								...
								$dir           = is_rtl() ? 'rtl' : 'ltr';
								?>
								<div class="sntravel-swiper-slider-inner sntravel-carousel-inner overflow-hidden">
						            <div class="sntravel-swiper-container" data-settings="<?php echo esc_attr($data_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
						                <div class="sntravel-swiper-wrapper swiper-wrapper">
						                	<?php 
						                	$k =  0;
						                	foreach ($gallery_ids as $ga_id) : 
						                		$k++;
						                		$attachment_info = Sntravel_Image::get_attachment_info( $ga_id );
												...
												<div class="sntravel-swiper-slide swiper-slide <?php echo esc_attr($item_cls) ?>" data-src="<?php echo esc_url($data_src) ?>" data-image-id="<?php echo esc_attr($ga_id) ?>" data-sub-html="<?php echo esc_attr( $sub_html ) ?>">
						        ...
						        <div class="sntravel-swiper-arrows nav-in-vertical arrow-on-hover">
					                <div class="sntravel-swiper-arrow sntravel-swiper-arrow-prev"><span class="sntravel-icon lnil lnil-chevron-left"></span></div>
            						<div class="sntravel-swiper-arrow sntravel-swiper-arrow-next"><span class="sntravel-icon lnil lnil-chevron-right"></span></div>
					            </div>
								<?php 
							}else{
							...
							<?php if( empty( $video_output) ): ?>
								<div class="sntravel-cursor-icon"><span class="sntravel-icon lnil lnil-plus"></span></div> 
							<?php endif; ?>
						</div>
					</div>
					<?php if ( ! empty( $gal_ids ) ) : 
						...
						<div class="product-gallery-img "> 
							<div class="product-gallery-carousel sntravel-swiper-slider">
								<div class="sntravel-swiper-slider-inner sntravel-carousel-inner overflow-hidden">
									<div class="swiper-container-thumbs sntravel-swiper-thumbs" data-settings="<?php echo esc_attr($data_thumb_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
				                		<div class="sntravel-swiper-wrapper swiper-wrapper">
											<?php foreach ($gallery_ids as $ga_id) : ?>
												<div class="sntravel-swiper-slide swiper-slide" data-image-id="<?php echo esc_attr($ga_id) ?>">
								...
								<div class="sntravel-swiper-arrows">
					                <div class="sntravel-swiper-arrow sntravel-swiper-arrow-prev"><span class="sntravel-icon lnil lnil-chevron-left"></span></div>
            						<div class="sntravel-swiper-arrow sntravel-swiper-arrow-next"><span class="sntravel-icon lnil lnil-chevron-right"></span></div>
					            </div>
							</div>
			...
			<?php if( $product_layout == 'layout-sticky'):
			 	$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
			 	if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'sntravel-product-sticky has-thumbs-gallery thumbs-vertical-left';
    				/*if ( $product_image_id = $product->get_image_id() ) { 
    					$gallery_ids = array_merge([$product_image_id], $gallery_ids) ;
    				}*/
    			}
    			$zoom_img_hover = sntravel()->get_theme_opt('zoom_img_hover', 'on');
    			$gal_display_type = isset($_GET['img-display-type']) ? sanitize_text_field( wp_unslash($_GET['img-display-type'])) : sntravel()->get_opt('product_grid_gal_display_type','all');
					...
					<?php endif; ?>
			 		<div class="product-main-img sntravel-light-gallery">
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
								?>	
				                <div class="sntravel-img-sticky-wrapper d-flex flex-column">
				                	<?php 
				                	$k =  0;
				                	foreach ($gallery_ids as $ga_id) : 
				                		$k++;
			...
			<?php if( $product_layout == 'layout-with-sidebar'):
				$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
    			if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'sntravel-product-swiper-slider sntravel-swiper-slider has-thumbs-gallery thumbs-horizontal-slider';
    				/*if ( $product_image_id = $product->get_image_id() ) { 
    					$gallery_ids = array_merge([$product_image_id], $gallery_ids) ;
    				}*/
    			}
    			...
    			<div class="<?php echo esc_attr( trim(implode(' ', $wrap_cls) )) ?>">
						            ...
						            <div class="sntravel-swiper-container" data-settings="<?php echo esc_attr($data_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
						                <div class="sntravel-swiper-wrapper swiper-wrapper">
						                	...
						                	foreach ($gallery_ids as $ga_id) : 
												...
												<div class="sntravel-swiper-slide swiper-slide zoom <?php echo esc_attr($item_cls) ?>" data-src="<?php echo esc_url($data_src) ?>" data-image-id="<?php echo esc_attr($ga_id) ?>" data-sub-html="<?php echo esc_attr( $sub_html ) ?>">
													<?php if(!empty( $video_output) && $video_light_gallery == 'off' && $k == 1 ): ?>
														<?php echo ''.$video_output; ?>
													<?php else: ?>
														<?php if($zoom_img_hover == 'on'): ?>
					...
					<?php if ( !empty( $gal_ids ) ): 
						...
						?>
						<div class="product-gallery-img"> 
							<div class="product-gallery-carousel sntravel-swiper-slider relative">
								<div class="sntravel-swiper-slider-inner sntravel-carousel-inner overflow-hidden">
									<div class="swiper-container-thumbs sntravel-swiper-thumbs" data-settings="<?php echo esc_attr($data_thumb_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
				                		<div class="sntravel-swiper-wrapper swiper-wrapper">
											<?php foreach ($gallery_ids as $ga_id) : ?>
								...
								<div class="sntravel-swiper-arrows nav-in-vertical">
					                <div class="sntravel-swiper-arrow sntravel-swiper-arrow-prev"><span class="sntravel-icon lnil lnil-chevron-left"></span></div>
            						<div class="sntravel-swiper-arrow sntravel-swiper-arrow-next"><span class="sntravel-icon lnil lnil-chevron-right"></span></div>
					            </div>
							</div>
			...
			<?php if( $product_layout == 'layout-full-width'):
			 	$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
			 	if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'sntravel-product-list-gallery';
    			}
    			$zoom_img_hover = sntravel()->get_theme_opt('zoom_img_hover', 'on');
    			$gal_display_type = isset($_GET['img-display-type']) ? sanitize_text_field( wp_unslash($_GET['img-display-type'])) : sntravel()->get_opt('product_grid_gal_display_type','all');
    			$display_type_cls = $gal_display_type == 'by-attribute' ? 'display-by-attribute' : 'display-all';
			 	...
			 	<div class="<?php echo esc_attr( trim(implode(' ', $wrap_cls) )) ?>">
			 		<div class="product-main-img sntravel-light-gallery">
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
								?>	
				                <div class="sntravel-img-list-wrapper d-flex flex-column">
				                	<?php 
				                	$k =  0;
				                	foreach ($gallery_ids as $ga_id) : 
				                		$k++;
			...
			<?php if( $product_layout == 'layout-grid-gallery'):
			 	$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
			 	if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'sntravel-product-grid-gallery';
    			}
    			$zoom_img_hover = sntravel()->get_theme_opt('zoom_img_hover', 'on');
    			$gal_display_type = isset($_GET['img-display-type']) ? sanitize_text_field( wp_unslash($_GET['img-display-type'])) : sntravel()->get_opt('product_grid_gal_display_type','all');
    			$display_type_cls = $gal_display_type == 'by-attribute' ? 'display-by-attribute' : 'display-all';
	    				...
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
								$row_cols_class = basilico_get_shop_loop_row_column_class([
								    'col_xs'  => sntravel()->get_opt('products_grid_gal_col_xs', '2'),
								    'col_sm'  => sntravel()->get_opt('products_grid_gal_col_sm', '2'),  
								    'col_md'  => sntravel()->get_opt('products_grid_gal_col_md', '2'), 
								    'col_lg'  => sntravel()->get_opt('products_grid_gal_col_lg', '2'),
								    'col_xl'  => sntravel()->get_opt('products_grid_gal_col_xl', '2'),  
								    'col_xxl' => sntravel()->get_opt('products_grid_gal_col_xxl', '2')
								]); 
								...
								?>	
				                <div class="sntravel-img-grid-wrapper row relative <?php echo esc_attr(implode(' ', $g_x_cls)) ?> <?php echo esc_attr(implode(' ', $g_y_cls)) ?> <?php echo esc_attr(implode(' ', $row_cols_class)) ?>">
				                	<?php 
				                	$k =  0;
				                	foreach ($gallery_ids as $ga_id) : 
				                		$k++;
    	...
    	public function get_loop_product_thumbnail($product,$args){ 
    		...
    		$shop_archive_hover_image = isset($args['shop_archive_hover_image']) ? $args['shop_archive_hover_image'] : sntravel()->get_theme_opt('shop_archive_hover_image', '0');
    		$thumb_class = '';
				...
				<?php if ( $has_hover_thumbnail ) { ?>
					<div class="product-hover-img sntravel-absoluted">
						...
						<?php if($shop_archive_hover_image == '2'): ?>
							<div class="product-loop-carousel sntravel-swiper-slider sntravel-theme-carousel">
								<div class="sntravel-swiper-slider-inner sntravel-carousel-inner overflow-hidden">
									<div class="sntravel-swiper-container" data-settings="<?php echo esc_attr($data_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
	                            		<div class="sntravel-swiper-wrapper swiper-wrapper">
											<?php foreach ($gallery_ids as $ga_id) : ?>
		...
		public function get_active_filter_remove_links( $filters ) {
			...
			foreach ( $filters as $filter_name => $value ) {
				...
				if ( ! empty( $taxonomy_name ) && taxonomy_exists( $taxonomy_name ) ) {
					...
					foreach ( $filter_terms as $key => $term_id ) {
						...
						$tooltip_text = isset( $taxonomy->labels->singular_name ) ? $taxonomy->labels->singular_name : __( 'Filter', 'sntravel' );

						$clear_links[] = [
							'url'     => $clear_link,
							'text'    => $term->name,
							'tooltip' => sprintf( __( 'Remove This %s', 'sntravel' ), $tooltip_text ),
					...
					foreach ( $filter_terms as $key => $term_slug ) {
						...
						$clear_links[] = [
							'url'     => $clear_link,
							'text'    => $term->name,
							'tooltip' => sprintf( __( 'Remove This %s', 'sntravel' ), $attribute_info->name ),
							'class'   => 'remove-filter-link',
						];
					}
					...
					foreach ( $filter_values as $key => $filter_value ) {
						...
						$clear_links[] = [
							'url'     => $clear_link,
							'text'    => sprintf( _n( '%s star', '%s stars', $filter_value, 'sntravel' ), $filter_value ),
							'tooltip' => sprintf( __( 'Remove This %s', 'sntravel' ), __( 'Rating', 'sntravel' ) ),
							'class'   => 'remove-filter-link',
						];
					}
					...
					$clear_links[] = [
						...
						'tooltip' => sprintf( __( 'Remove This %s', 'sntravel' ), __( 'Highlight', 'sntravel' ) ),
						'class'   => 'remove-filter-link',
					];
				}*/
			}
			...
			if ( isset( $filters['min_price'] ) && isset( $filters['max_price'] ) ) {
				...
				$clear_links[] = [
					...
					'tooltip' => sprintf( __( 'Remove This %s', 'sntravel' ), __( 'Price', 'sntravel' ) ),
					'class'   => 'remove-filter-link',
				];
			}
 
			$output = '<div class="sntravel-active-filters-inner">';
			foreach ( $clear_links as $clear_link ) {
				...
				$tooltip_text = ! empty( $clear_link['tooltip'] ) ? $clear_link['tooltip'] : esc_html__( 'Remove This', 'sntravel' );
			...
			if ( ! empty( $clear_links ) ) {
				$clear_all_links = [
					...
					'text'  => '<span class="lnil lnil-reload"></span>'.esc_html__( 'Reset All', 'sntravel' ),
					'class' => 'reset-all',
					'tooltip' => esc_html__( 'Remove This', 'sntravel' ),
				];
  		...
  		public function get_category_list_filter($wg_id = '', $settings = []){
			...
			?>
			<div id="<?php echo esc_attr($wg_id) ?>" class="widget sntravel-widget sntravel-widget-product-categories sntravel-widget-wc-layered-nav">
			...
			$chosen_terms   = $this->get_chosen_terms( 'filter_product_cat' );
  			echo '<ul class="' . esc_attr( implode( ' ', $class ) ) . '">';
        		...
        		foreach ( $terms as $term ) {
					...
					$item_class = [ 'sntravel-layered-nav-term sntravel-list-item' ];
		...
		public function get_sortby_list_filter($wg_id = '', $settings = []){
			extract($settings);
  			$orderby_options = array(
				'popularity' => esc_html__( 'Popularity', 'sntravel' ),
				'rating'     => esc_html__( 'Average Rating', 'sntravel' ),
				'sale'       => esc_html__( 'Sale', 'sntravel' ),
				'date'       => esc_html__( 'New In', 'sntravel' ),
				'price'      => esc_html__( 'Price Low to High', 'sntravel' ),
				'price-desc' => esc_html__( 'Price High to Low', 'sntravel' ),
            );
            $base_link = $this->get_shop_active_filters_url();
            ...
            ?>
            <div id="<?php echo esc_attr($wg_id) ?>" class="widget sntravel-widget sntravel-widget-product-orderby sntravel-widget-wc-layered-nav">
                <div class="widget-content">
                	<?php if(!empty($wg_title)): ?>
                		<h4 class="widget-title"><span><?php echo esc_html($wg_title) ?></span></h4>
                	<?php endif; ?>
                    <div class="widget-content-inner <?php echo esc_attr($content_class) ?>">
                    	<ul class="sortby-filter">
				            ...
				            foreach ( $orderby_options as $id => $name ){
								...
								$item_class = [ 'sntravel-layered-nav-term sntravel-list-item orderby' ];
								$link_class = 'filter-link';
  						...
  						<form method="get" action="<?php echo esc_url( $form_action ); ?>">
			                <div class="price-filter-widget price_slider_wrapper">
			                    ...
			                    <div class="price_slider_amount" data-step="<?php echo esc_attr( $step ); ?>"> 
			                    	<label class="screen-reader-text" for="min_price"><?php esc_html_e( 'Min price', 'sntravel' ); ?></label>
									<input type="text" id="min_price" name="min_price" value="<?php echo esc_attr( $current_min_price ); ?>" data-min="<?php echo esc_attr( $min_price ); ?>" placeholder="<?php echo esc_attr__( 'Min price', 'sntravel' ); ?>" />
									<label class="screen-reader-text" for="max_price"><?php esc_html_e( 'Max price', 'sntravel' ); ?></label>
									<input type="text" id="max_price" name="max_price" value="<?php echo esc_attr( $current_max_price ); ?>" data-max="<?php echo esc_attr( $max_price ); ?>" placeholder="<?php echo esc_attr__( 'Max price', 'sntravel' ); ?>" />
				                    <button type="submit" class="sntravel-btn btn-outline button"><?php echo esc_html__( 'Filter', 'sntravel' ); ?></button>
		...
		public function get_attribute_filter($wg_id = '', $settings = []){
			...
			?>
			<div id="<?php echo esc_attr($wg_id) ?>" class="widget sntravel-widget widget_layered_nav woocommerce-widget-layered-nav sntravel-widget-layered-nav">
                <div class="widget-content">
                	<?php if(!empty($wg_title)): ?>
                		<h4 class="widget-title"><span><?php echo esc_html($wg_title) ?></span></h4>
                	<?php endif; ?>
									...
									if ( 'swatches' === $list_style ) :
										switch ( $attr_info->type ) :
											case 'color':
												...
												$item_link_class .= ' sntravel-ttip tt-top';
					...
					if( $enable_load_more == '1' && count($term_counts) > $items_loadmore_display){  
						echo '<span class="sntravel-toogle-more" data-display="'.esc_attr($items_loadmore_display).'" data-less="'.esc_attr__( 'Less', 'sntravel' ).'" data-more="'.esc_attr__( 'More', 'sntravel' ).'" ><span class="text">' . esc_html__( 'More', 'sntravel' ) . '</span><span class="sntravel-icon lnil lnil-chevron-down"></span></span>';
					}
                    ?>
                </div>
            </div>
		...
		public function get_recent_viewed_products(){
			...
			$viewed_pids = [];
			$posts_per_page     = (int)sntravel()->get_theme_opt('product_recent_viewed_per_page', 8);
		...
		public function get_cross_sell_product(){
			...
			$cross_sells_ids = sntravel()->get_page_opt('product_cross_sell_ids', []);
