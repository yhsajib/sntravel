...
class Pxl_Log
{
	...
	{  
		...
		$this->log_folder = 'sntravelart_log';
		$this->imported = 0;
		add_action( 'wp_ajax_sntravelart_progress_imported', array($this, 'getProgress'), 10, 1 );
		add_action( 'wp_ajax_sntravelart_total_imported', array($this, 'getImported'), 10, 1 );
		add_action( 'wp_ajax_sntravelart_reset_logs', array($this, 'resetFiles'), 10, 1 );
		add_action( 'admin_footer', array( $this, 'loadingTpl'), 10, 1 );
	}
	  
	public function loadingTpl(){
		echo '
		  	<div class="sntravel-demo-loader sntravel-imp-popup-wrap">
				<div class="sntravel-imp-loader">	
					<div class="loader"></div>
					<h4>'.esc_html__( 'Downloading...', SNTRAVEL_TEXT_DOMAIN ).'</h4>
				</div> 
	    	</div>';
	    echo '<div class="sntravel-demo-error-confirm">';
		    echo '<div class="confirm-inner">';
		    echo '<h4>'.esc_html__( 'Error', SNTRAVEL_TEXT_DOMAIN ).'</h4>';
		    echo '<div class="message"></div>';
		    ?>
		    <div class="sntravel-form-upload-demo text-center">
				<form enctype='multipart/form-data' action='' method='post'>
					<h4><?php esc_html_e( 'Upload demo data zip file', SNTRAVEL_TEXT_DOMAIN )?></h4>
					<p><?php esc_html_e( 'Please search file in full-package -> demo-data was downloaded from Themeforest.', SNTRAVEL_TEXT_DOMAIN )?> <span><?php esc_html_e( 'Or', SNTRAVEL_TEXT_DOMAIN )?></span> <span class="link-download-demo-manual"></span> <span><?php esc_html_e( 'for download file', SNTRAVEL_TEXT_DOMAIN )?></span></p>
					<div class="upload-demo-fields">
						...
						<button type="button" class="btn btn-upload btn-default">
							<span class="btn-text"><?php echo esc_html__( 'Upload File', SNTRAVEL_TEXT_DOMAIN ); ?></span>
		    ...
		    <?php 
		    echo '<div class="confirm-footer text-right"><button type="button" class="btn btn-default">'.esc_html__( 'Cancel', SNTRAVEL_TEXT_DOMAIN ).'</button></div>';
		    echo '</div>';
	    echo '</div>';
	}
