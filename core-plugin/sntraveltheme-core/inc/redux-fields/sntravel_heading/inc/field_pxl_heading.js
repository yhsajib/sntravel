			...
			$( 'body' ).on(
				...
				function( e, variable ) {
					...
					$.each(
						...
						function( child ) {
							var sntravel_heading_div;
							var show          = false;
							var childFieldset = $( '#' + redux.optName.args.opt_name + '-' + child );

							if( childFieldset.closest('.redux-container-sntravel_heading').length > 0 ){
								sntravel_heading_div = childFieldset.closest('.form-table').next('.indent-section-container');
								if(sntravel_heading_div.length <= 0){
									sntravel_heading_div = childFieldset.closest('.form-table').next('.redux-section-collapse-wrapper');
								}
							}
							...
							if ( true === show ) {
								if (typeof sntravel_heading_div !== 'undefined') {
									sntravel_heading_div.fadeIn(
										300,
										function() {
