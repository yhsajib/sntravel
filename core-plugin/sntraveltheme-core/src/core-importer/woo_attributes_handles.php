...
if (!class_exists('SNTRAVEL_Woo_Attributes_Handle')) {
    class SNTRAVEL_Woo_Attributes_Handle {
        public function __construct() {
            add_action('init', array($this, 'sntravel_import_woo_term'), 29);
        }

        function sntravel_import_woo_term(){
            $upload_dir = wp_upload_dir();
            $current_id = get_option('sntravel_import_demo_id',true);
            //$term_imported = get_option('sntravel_woo_term_imported',"null");
            $folder_name = sanitize_title($current_id);
            $folder_dir = $upload_dir['basedir'].DIRECTORY_SEPARATOR.'sntravelart_temp'.DIRECTORY_SEPARATOR.$folder_name.DIRECTORY_SEPARATOR; 
            //if($term_imported === "not_imported"){
            $this->sntravel_woo_attributes_term_import($folder_dir . 'woo_attributes.json');
            //}
        }

        function sntravel_woo_attributes_term_import($file){
            if (file_exists($file) && class_exists('WooCommerce')) {
                update_option("sntravel_woo_term_imported","imported");
    ...
    new SNTRAVEL_Woo_Attributes_Handle(); 
}
