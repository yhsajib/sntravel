    ...
    if (typeof(one_page_options) != "undefined") {
        ...
        $('.is-one-page').on('click', function (e) {
            ...
            if ($(_id_data).length === 1) {
                ...
                $('.sntravel-onepage-active').closest('li.menu-item').removeClass('active');
                $('.sntravel-onepage-active').removeClass('sntravel-onepage-active');
                _this.closest('li.menu-item').addClass('active');
                _this.addClass('sntravel-onepage-active');
            ...
            var waypoint = new Waypoint({
                ...
                handler: function(direction) {
                    if(direction === 'down'){
                        ...
                        $(item).closest('ul').find('.sntravel-onepage-active').removeClass('sntravel-onepage-active');
                        $(item).closest('li.menu-item').addClass('active');
                        $(item).addClass('sntravel-onepage-active');
                    }
                    else if(direction === 'up'){
                        var prev = $(item).parent().prev().find('.is-one-page');
                        $(item).closest('li.menu-item').removeClass('active');
                        $(item).removeClass('sntravel-onepage-active');
                        if(prev.length > 0){
                            prev.closest('li.menu-item').addClass('active');
                            prev.addClass('sntravel-onepage-active');
                        }
                    }
                    else{
                        console.log('abc');
        ...
        $('.is-one-page').on('click', function (e) {
            ...
            if ($(_id_data).length === 1) {
                ...
                $(this).addClass('sntravel-onepage-active');
                $('html, body').stop().animate({ scrollTop: _target.offset().top - _offset }, one_page_options.speed);   
                return false;
            } else {
        ...
        var sntravel_section_header = '.sntravel-section-header';
        $('.sntravel-section-header').each(function() {
                ...
                var waypoint = new Waypoint({
                    ...
                    handler: function(direction) {
                        if(direction === 'down'){
                            ...
                            $header_type_onepage.removeClass('sntravel-onepage-active');
                            $(item).closest('li.menu-item').addClass('active');
                            $(item).addClass('sntravel-onepage-active');
                        }
                        else if(direction === 'up'){
        ...
        var op_menu_target_mobile = '.sntravel-mobile-menu .is-one-page';
            ...
            var waypoint = new Waypoint({
                ...
                handler: function(direction) {
                    if(direction === 'down'){
                        ...
                        $(op_menu_target_mobile).removeClass('sntravel-onepage-active');
                        $(item).closest('li.menu-item').addClass('active');
                        $(item).addClass('sntravel-onepage-active');
                    }
                    else if(direction === 'up'){
