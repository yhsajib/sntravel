<?php
/**
 * The Sntravel_Register initiate the theme engine
 */

if( !defined( 'ABSPATH' ) ) 
	exit; // Exit if accessed directly

class Sntravel_Register {
	...
	function admin_notice() {
		echo '<div class="notice"><p>'.esc_html__( 'Purchase code is invalid. Need a license for activation', 'sntravel' ).'</p></div>';
	}
	...
	function messages($merlin = false){
		...
		if ( ! $purchase_code ){
			?>
			<div class="sntravel-dsb-box-head-inner">
				<h6><?php echo esc_html__( 'Register License', 'sntravel' ) ?></h6>
			</div>
			<?php 
			$this->form();
			?>
			<div class="sntravel-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'sntravel' ); ?></a>
			</div>
			<?php 
		}else{  
	...
	function check_license($merlin) {
		...
		if ( false === $license_data->success ) {
			switch ( $license_data->error ) {
				case 'missing':
					$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'sntravel' );
				break;
				case 'item_name_mismatch':
					$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'sntravel' ), $this->theme_name );
				break;
				case 'license_exists':
					$message = esc_html__( 'Your license is not active for this URL.', 'sntravel' );
				break;
				default:
					$message = esc_html__( 'An error occurred, please try again.', 'sntravel' );
				break;
			}
			?>
	...
	function form() {
		...
		?>
		<form action="options.php" method="post" class="sntravel-dsb-register-form">
			<?php settings_fields( $this->theme_slug . '-license' ); ?>
			<input id="<?php echo esc_attr($this->theme_slug)?>_purchase_code" name="<?php echo esc_attr($this->theme_slug)?>_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter your purchase code', 'sntravel' ); ?>">
			<input type="submit" class="res-purchase-code" value="<?php esc_attr_e( 'Register your purchase code', 'sntravel' ) ?>">
		</form>
		<?php
	}
