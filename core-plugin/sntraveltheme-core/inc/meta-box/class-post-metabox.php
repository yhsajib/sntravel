...
class SNTRAVEL_Post_Metabox
{
    ...
    function __construct()
    {
        ...
        do_action('sntravel_post_metabox_register', $this);
        ...
        if (!empty($this->panels)) {
            ...
            add_action('hidden_meta_boxes', array($this, 'sntravel_set_user_metaboxes'));
        }
    }
    ...
    {
        $this->default_args['opt_name'] = isset($this->default_args['opt_name']) ? $this->default_args['opt_name'] . '_post_metabox' : 'sntravel_post_metabox';
        $this->default_args['display_name'] = esc_html__('Settings', SNTRAVEL_TEXT_DOMAIN);
    ...
    function add_section($post_type, $section = array()){
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$post_type])) {
                trigger_error(esc_html__('The field with id', SNTRAVEL_TEXT_DOMAIN) . ' ' . esc_html($field['id']) . ' ' . esc_html__('for post type', SNTRAVEL_TEXT_DOMAIN) . ' ' . esc_html($post_type) . ' ' . esc_html__('is already registered.', SNTRAVEL_TEXT_DOMAIN));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $post_type)) {
                trigger_error(esc_html__('Section', SNTRAVEL_TEXT_DOMAIN) . ' ' . esc_html($field['id']) . ' ' . esc_html__('for post type', SNTRAVEL_TEXT_DOMAIN) . ' ' . esc_html($post_type) . ' ' . esc_html__('is already registered.', SNTRAVEL_TEXT_DOMAIN));
    ...
    function admin_init()
    {
        ...
        if ('post.php' == $pagenow) {
            $this->notices = get_transient('sntravel-post-metabox-transients');

            // If the transient exists then we override it with an empty value
            if (false !== $this->notices) {
                set_transient('sntravel-post-metabox-transients', '');
            } else {
    ...
    function add_meta_boxes()
    {
        ...
        foreach ($this->panels as $post_type => $panel) {
            if ($post_type !== 'post') {
                ...
                if (in_array(str_replace('sntravel_pf_', '', $new_post_type), $post_formats) !== false) {
                    $new_post_type = 'post';
                    $priority = 'core';
                }
                add_meta_box(
    ...
    function save_meta_boxes($post_id, $post)
    {
        ...
        do_action('sntravel_pre_save_post');

        if (!isset($_POST['sntravel_mtb_post_nonce']) || !wp_verify_nonce(sanitize_key(wp_unslash($_POST['sntravel_mtb_post_nonce'])), 'sntravel_mtb_post_nonce_action')) {
            return;
        }
        $opt_name = '';
        $data_to_save = [];
        ...
        if (!empty($post_format)) {
            $sntravel_pf_panel = 'sntravel_pf_' . $post_format;
            $post_format_type = !empty($_POST['post_format_' . $post_format]) ? $_POST['post_format_' . $post_format] : '';
            if (in_array($sntravel_pf_panel, $this->post_types) && !empty($_POST[$this->panels[$sntravel_pf_panel]['args']['opt_name']]) && !empty($post_format_type)) {
                $opt_name = $this->panels[$sntravel_pf_panel]['args']['opt_name'];
                $sections_post_format = $this->get_opt_sections($sntravel_pf_panel);
                $args_post_format = $this->get_opt_args($sntravel_pf_panel);
                $data_to_save_pfm = array();
                $data_to_compare_pfm = $this->get_opt_defaults($sntravel_pf_panel);
        ...
        if (!empty($redux->errors) || !empty($redux->warnings)) {
            ...
            set_transient('sntravel-post-metabox-transients', $notices);
        }
    ...
    function admin_notices()
    {
        ...
        if (!empty($this->notices['errors'])) {
            ...
            printf(
                ...
                esc_html__('error(s)', SNTRAVEL_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', SNTRAVEL_TEXT_DOMAIN)
            );
            echo '</p></div>';
        }
        ...
        if (!empty($this->notices['warnings'])) {
            ...
            printf(
                ...
                esc_html__('warning(s)', SNTRAVEL_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', SNTRAVEL_TEXT_DOMAIN)
            );
    ...
    function sntravel_set_user_metaboxes($hidden)
    {
    ...
    function generate_panel($post, $args)
    {
        ...
        wp_nonce_field('sntravel_mtb_post_nonce_action', 'sntravel_mtb_post_nonce');
...
//        die();
        ...
        wp_enqueue_style('sntravel-metabox', SNTRAVEL_URL . '/assets/css/metabox' . Redux_Functions::isMin() . '.css', array(), self::$version, 'all');
        wp_enqueue_script('sntravel-metabox', SNTRAVEL_URL . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array(
            'jquery',
            'redux-js'
        ), self::$version, 'all');
    ...
    {
        return SNTRAVEL_PATH . 'inc/meta-box/templates';
    }
    ...
    protected function generate_output_css( $post, $options ){
        ...
        printf(
            ...
            esc_attr('sntravel-' . $post->post_type . '-dynamic-css'),
            $redux->outputCSS
        );
    }
}