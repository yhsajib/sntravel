...
if(!empty($info_items)) : ?>
    <div id="<?php echo esc_attr($html_id); ?>" class="sntravel-single-info e-sidebar-widget <?php echo esc_attr($el_style); ?>">
                                ...
                                case 'post_categories':
                                    switch(get_post_type()) {
                                        ...
                                        case 'sntravel-portfolio':
                                            the_terms('', 'sntravel-portfolio-category', '', '&nbsp-&nbsp', '');
                                            break;
                                    }
                                    break;
                                case 'post_tags':
                                            ...
                                            if ($posttags) {
                                                foreach ($posttags as $key => $tag) {
                                                    ...
                                                    if ($key !== count($posttags) - 1) {
                                                        echo esc_html(' - ', 'sntravel');
                                                    }
                                                }
                                            };
                                            break;
                                        case 'sntravel-portfolio':
                                            the_terms('', 'sntravel-portfolio-tag', '', '&nbsp-&nbsp', '');
                                            break;
                                    }
                                    break;
                            }
        ...
        <?php if($show_social) : ?>
            <div class="info-item d-flex">
                <div class="inner-text">
                    <span class="info-text"><?php sntravel()->blog->get_post_share(); ?></span>
                </div>
            </div>
        <?php endif; ?>
    </div>
