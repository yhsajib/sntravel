...
if(!class_exists('Pxl_Download_Demo')){
    require_once SNTRAVEL_PATH . 'src/core-importer/download-demo.php';
}
if(!class_exists('Pxl_Log')){
    require_once SNTRAVEL_PATH . 'src/core-importer/import-log.php';
} 
class Pxl_Importer{
    ...
    public function __construct() {
		...
		//$this->theme_option_name       = 'sntravel_theme_options';
        ...
        add_action( 'wp_ajax_sntravelart_import_start', array($this, 'ajax_import_start'), 10, 1 );
        add_action( 'wp_ajax_sntravelart_import_media', array($this, 'ajax_import_media'), 10, 1 );
        add_action( 'wp_ajax_sntravelart_import_content', array($this, 'ajax_import_content'), 10, 1 );
        add_action( 'wp_ajax_sntravelart_import_theme_options', array($this, 'ajax_import_theme_options'), 10, 1 );
        add_action( 'wp_ajax_sntravelart_import_widgets', array($this, 'ajax_import_widgets'), 10, 1 );
        add_action( 'wp_ajax_sntravelart_import_slider', array($this, 'ajax_import_slider'), 10, 1 );
        add_action( 'wp_ajax_sntravelart_import_settings', array($this, 'ajax_import_settings'), 10, 1 );
        add_action( 'wp_ajax_sntravelart_import_finish', array($this, 'ajax_import_finish'), 10, 1 );
    ...
    public function ajax_import_start(){
        ...
        do_action('sntravel-import-start', $demo);
        ...
        if( $skip_posts == 'no'){
            $this->sntravel_import_truncate_tables();
        }
    ...
    public function ajax_import_media() {
        ...
        if( empty($zip_file) || !is_file( $zip_file )){
            ...
            wp_die( esc_html__( 'Error: Attachment file not found!', SNTRAVEL_TEXT_DOMAIN ) );
        }
        ...
        if( empty($file) || !is_file( $file )){
            ...
            wp_die(
                esc_html__( 'The XML file containing the dummy content is not available or could not be read .. You might want to try to set the file permission to chmod 755.<br/>If this doesn\'t work please use the Wordpress importer and import the XML file (should be located in your download .zip: Sample Content folder) manually', SNTRAVEL_TEXT_DOMAIN ),'',array( 'back_link' => true )
            );
        }
        ...
        if($importer_error){
            ...
            wp_die( esc_html__( 'Error on import media', SNTRAVEL_TEXT_DOMAIN ) ); 
        } else { 
            $wp_import = new SNTRAVEL_Import();  
            ...
            echo esc_html__('Import media successfully!', SNTRAVEL_TEXT_DOMAIN );
        } 
    	wp_die();
    }
    ...
    public function ajax_import_content() {
        ...
        if($importer_error){
            ...
            wp_die( esc_html__( 'Error on import content', SNTRAVEL_TEXT_DOMAIN ) ); 
        } else { 
            $wp_import = new SNTRAVEL_Import();  
            $zip_file = $this->media_file_zip_path; // $this->demo_files_path.$this->media_file_zip;
            $attachment = !is_file( $zip_file ) ? $this->sntravel_import_add_placeholder_image() : null;      
            $wp_import->import( $file, $attachment );
            $this->log->putContent('Content imported successfully', true, true); 
            $this->log->putContent('Content imported successfully |');
            echo esc_html__('Import Content successfully!', SNTRAVEL_TEXT_DOMAIN );
	    	...
	    	if (file_exists($woo_atts_file) && class_exists('WooCommerce')) {
	            ...
	            if(isset($atts_data["tax"]) && count($atts_data["tax"])){
                    ...
                    do_action( 'sntravel_before_import_product_attribute' );
	            ...
	            update_option("sntravel_woo_term_imported","not_imported");
	        }
            // import term meta
    ...
    public function ajax_import_theme_options() {
        ...
        if(empty($file) || !is_file( $file )){
            ...
            wp_die(
                esc_html__( 'The theme options json file containing the dummy content is not available or could not be read .. You might want to try to set the file permission to chmod 755.', SNTRAVEL_TEXT_DOMAIN ),'',array( 'back_link' => true )
            );
        }
        ...
        if ( empty( $data ) || ! is_array( $data ) ) {
            ...
            wp_die( esc_html__( 'Theme options import data could not be read. Please try a different file.', SNTRAVEL_TEXT_DOMAIN ));
        }


        $_replaces = apply_filters('sntravel_import_replace_theme_options', array());
        ...
        if(!empty($setting_file) || is_file( $setting_file )){
            ...
            if(!empty( $settings['opt-name']))
                $this->theme_option_name = apply_filters('sntravel_ie_options_name', $settings['opt-name']);
            else
                $this->theme_option_name = apply_filters('sntravel_ie_options_name', 'sntravel_theme_options');
        }else{
            $this->theme_option_name = apply_filters('sntravel_ie_options_name', 'sntravel_theme_options');
        }
        ...
        echo esc_html__('Import theme options successfully!', SNTRAVEL_TEXT_DOMAIN ); 
    	wp_die();
    }
    ...
    public function ajax_import_widgets() {
        ...
        if(empty($file) || !is_file( $file )){
            ...
            wp_die(
                esc_html__( 'The widgets file containing the dummy content is not available or could not be read .. You might want to try to set the file permission to chmod 755.', SNTRAVEL_TEXT_DOMAIN ),'',array( 'back_link' => true )
            );
        }
	    ...
	    $this->sntravel_import_widgets_import_data($data);
         
        $this->log->putContent('Widgets imported successfully', true, true); 
        $this->log->putContent('Widgets imported successfully |');
        echo esc_html__('Import widgets successfully!', SNTRAVEL_TEXT_DOMAIN ); 
    	wp_die();
    }
    ...
    public function ajax_import_slider(){
    	...
    	if(class_exists('RevSliderSlider')) {
		    ...
		    if(is_dir($folder)){
		        ...
		        echo esc_html__('Import slider data successfully', SNTRAVEL_TEXT_DOMAIN ); 
		        return $log;
		    }
    		 
    	} else {
    		echo esc_html__( 'Failed to import slider data, Please make sure to install and activate slider revolution plugin first', SNTRAVEL_TEXT_DOMAIN );
    ...
    public function ajax_import_settings(){
    	...
    	if(empty($file) || !is_file( $file )){
            ...
            wp_die(
                esc_html__( 'The theme setting json file containing the dummy content is not available or could not be read .. You might want to try to set the file permission to chmod 755.', SNTRAVEL_TEXT_DOMAIN ),'',array( 'back_link' => true )
            );
        }
	    ...
	    echo esc_html__('Import settings successfully', SNTRAVEL_TEXT_DOMAIN ); 
	    wp_die();
    }
    ...
    public function ajax_import_finish(){
        ...
        if (file_exists($this->demo_info_files_path)){ 
            ...
            if(!empty($info_demo['old_domain'])){
                ...
                if (file_exists($this->demo_files_path.'extra-tables.json')) {
                    $extra_tables = apply_filters('sntravel_ie_extra_tables', array());
        ...
        do_action('sntravel-import-finish', $demo);

        update_option('sntravel_import_demo_id',$demo);

        if (!empty($crop_img) && $crop_img === 'yes') {
            //set_time_limit(0);
            $this->sntravel_import_crop_images();
        }

        //$this->sntravel_import_clear_tmp($temp_dir);
	...
	function sntravel_import_add_placeholder_image(){

        $attachment_exists = get_page_by_title(esc_html__('Image Placeholder', SNTRAVEL_TEXT_DOMAIN), OBJECT, 'attachment');
        ...
        $_default_image = apply_filters('sntravel_import-placeholder-image', SNTRAVEL_URL . '/assets/img/df_import_placeholder.jpg');
        ...
        $attachment = array(
            ...
            'post_title'     => esc_html__('Image Placeholder', SNTRAVEL_TEXT_DOMAIN),
            'post_status'    => 'inherit'
        );
    ...
    function sntravel_import_widgets_import_data($data){
	    ...
	    // Hook before import
		//    do_action('sntravel_before_widget_import');
		$data = apply_filters('sntravel_import_widgets_data', $data);


	    // Get all available widgets site supports
	    $available_widgets = $this->sntravel_import_available_widgets();
	    ...
	    foreach ($data['sidebars'] as $sidebar_id => $widgets) {
	        ...
	        if (isset($wp_registered_sidebars[$sidebar_id])) {
	            ...
	            $sidebar_message = esc_html__('Sidebar does not exist in theme (using Inactive)', SNTRAVEL_TEXT_DOMAIN);
	        }
	        ...
	        foreach ($widgets as $widget_instance_id => $widget) {
	            ...
	            if (!$fail && !isset($available_widgets[$id_base])) {
	                ...
	                $widget_message = __('Site does not support widget', SNTRAVEL_TEXT_DOMAIN); // explain why widget not imported
	            }

	            // Filter to modify settings object before conversion to array and import
	            // Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below)
	            // Ideally the newer sntravel_ie_widget_settings_array below will be used instead of this
	            $widget = apply_filters('sntravel_ie_widget_settings', $widget); // object
	            ...
	            // This is preferred over the older sntravel_ie_widget_settings filter above
	            // Do before identical check because changes may make it identical to end result (such as URL replacements)
	            $widget = apply_filters('sntravel_import_widget_settings_array', $widget);
	            ...
	            if (!$fail && isset($widget_instances[$id_base])) {
	                ...
	                foreach ($single_widget_instances as $check_id => $check_widget) {
	                    ...
	                    if (in_array("$id_base-$check_id", $sidebar_widgets) && (array)$widget == $check_widget) {
	                        ...
	                        $widget_message = __('Widget already exists', SNTRAVEL_TEXT_DOMAIN); // explain why widget not imported
	            ...
	            if (!$fail) {
	                ...
	                do_action('sntravel_after_widget_import', $after_widget_import);

	                // Success message
	                if ($sidebar_available) {
	                    $widget_message_type = 'success';
	                    $widget_message = __('Imported', SNTRAVEL_TEXT_DOMAIN);
	                } else {
	                    $widget_message_type = 'warning';
	                    $widget_message = __('Imported to Inactive', SNTRAVEL_TEXT_DOMAIN);
	                }
	            ...
	            $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = !empty($widget['title']) ? $widget['title'] : __('No Title', SNTRAVEL_TEXT_DOMAIN); // show "No Title" if widget instance is untitled
	...
	function sntravel_import_available_widgets(){
	...
	function sntravel_import_crop_images(){
    ...
    function sntravel_import_truncate_tables(){
        $tables = apply_filters('sntravel_import_truncate_tables', [
            'posts',
            'postmeta',
            'terms',
            'termmeta',
    ...
    function sntravel_import_clear_tmp($dir){
        ...
        foreach (scandir($dir) as $item) {
            ...
            if (!$this->sntravel_import_clear_tmp($dir . "/" . $item, false)) {
                chmod($dir . "/" . $item, 0777);
                if (!$this->sntravel_import_clear_tmp($dir . "/" . $item, false)) return false;
            };
        }
        return rmdir($dir);
    }
