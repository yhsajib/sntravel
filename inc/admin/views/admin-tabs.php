<?php 
	$dashboard_page_url = admin_url( 'admin.php?page=sntravelart' );
	if( isset( $_GET['page'] ) && 'sntravelart' === sanitize_text_field($_GET['page']) ) {
		$dashboard_page_url = '';
	}
	$plugin_page_url = admin_url( 'admin.php?page=sntravelart-plugins' );
	$import_demos_page_url = admin_url( 'admin.php?page=sntravelart-import-demos' );

	$sntravel_server_info = apply_filters( 'sntravel_server_info', 
		[
			'video_url' => '',
			'demo_url' => '',
			'docs_url' => '',
...
<nav class="sntravel-dsb-menubar">
	...
	<div class="sntravel-dsb-menu">
		<ul class="sntravel-dsb-menu-left">
			<li class="<?php echo ( isset( $_GET['page'] ) && 'sntravelart' === sanitize_text_field($_GET['page']) ) ? 'is-active' : ''; ?>">
				<a href="<?php echo esc_attr($dashboard_page_url); ?>">
					<span><?php echo sprintf( esc_html__( '%s Dashboard', 'sntravel' ), sntravel()->get_name()); ?></span>
				</a>
			</li>
			<li class="<?php echo ( isset( $_GET['page'] ) && 'sntravelart-plugins' === sanitize_text_field($_GET['page']) ) ? 'is-active' : ''; ?>">
				<a href="<?php echo esc_url($plugin_page_url); ?>">
					<span><?php esc_html_e( 'Install Plugins', 'sntravel' ); ?></span>
				</a>
			</li>
			<li class="<?php echo ( isset( $_GET['page'] ) && 'sntravelart-import-demos' === sanitize_text_field($_GET['page']) ) ? 'is-active' : ''; ?>">
		...
		<ul class="sntravel-dsb-menu-right">
			<li>
				<a href="<?php echo esc_url($sntravel_server_info['video_url']) ?>" target="_blank">
					<span><?php esc_html_e( 'Videos tutorial', 'sntravel' ); ?></span>
				</a>
			</li>
			<li>
				<a href="<?php echo esc_url($sntravel_server_info['support_url']) ?>" target="_blank">
					<span><?php esc_html_e( 'Support system', 'sntravel' ); ?></span>
				</a>
			</li>
			<li>
				<a href="<?php echo esc_url($sntravel_server_info['demo_url']) ?>" target="_blank">
					<span><?php esc_html_e( 'Live Demo', 'sntravel' ); ?></span>
				</a>
			</li>
			 
			<li>
				<a href="<?php echo esc_url($sntravel_server_info['docs_url']) ?>" target="_blank">
