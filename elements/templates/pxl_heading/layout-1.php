...
//     }
// }
$widget->add_render_attribute( 'wrap-heading', 'class', 'sntravel-heading-wrap d-flex layout1');
...
if(!empty($settings['title_split_text_anm'])){
    $widget->add_render_attribute( 'large-title', 'class', 'sntravel-split-text '.$settings['title_split_text_anm']);
}
if ( $settings['title_animation'] ) {
    $widget->add_render_attribute( 'large-title', 'class', 'sntravel-animate sntravel-invisible animated-'.$settings['title_animation_duration']);
...
if ( $settings['sub_title_animation'] ) {
    $widget->add_render_attribute( 'sub-title', 'class', 'sntravel-animate sntravel-invisible animated-'.$settings['sub_title_animation_duration']);
...
if(!empty($settings['subtitle_split_text_anm'])){
    $widget->add_render_attribute( 'sub-title-text', 'class', 'sntravel-split-text '.$settings['subtitle_split_text_anm']);
}

$widget->add_render_attribute( 'description', 'class', 'heading-description');
if ( $settings['description_animation'] ) {
    $widget->add_render_attribute( 'description', 'class', 'sntravel-animate sntravel-invisible animated-'.$settings['description_animation_duration']);
...
if(!empty($settings['description_split_text_anm'])){
    $widget->add_render_attribute( 'description', 'class', 'sntravel-split-text '.$settings['description_split_text_anm']);
}
...
?>
<div <?php sntravel_print_html($widget->get_render_attribute_string( 'wrap-heading' )); ?>>
    <div class="sntravel-heading-inner">
        <?php if($settings["title_highlighted_style"] == 'style-3') : ?>
            <span class="icecream-heading"></span>
        <?php endif; ?>
        <?php if(!empty($sub_title)) : ?>
            <div <?php sntravel_print_html($widget->get_render_attribute_string( 'sub-title' )); ?>>
                <span <?php sntravel_print_html($widget->get_render_attribute_string( 'sub-title-text' )); ?>><?php sntravel_print_html(nl2br($sub_title)); ?></span>
            </div>
        <?php endif; ?>
        <<?php echo esc_attr($title_tag); ?> <?php sntravel_print_html($widget->get_render_attribute_string( 'large-title' )); ?>>
            <?php if (!empty( $settings['link']['url']) ) : ?>
                <<?php echo implode( ' ', [ $inner_tag, $link_attributes ] ); ?>>
            <?php endif; ?>
            <?php echo wp_kses_post(nl2br($title)); ?>
            <?php if (!empty( $settings['link']['url']) ) : ?>
                </<?php sntravel_print_html($inner_tag); ?>>
            <?php endif; ?>
        </<?php echo esc_attr($title_tag); ?>>
        <?php if(!empty($description)) : ?>
            <div <?php sntravel_print_html($widget->get_render_attribute_string( 'description' )); ?>>
                <span><?php sntravel_print_html(nl2br($description)); ?></span>
            </div>
        <?php endif; ?>
    </div>
</div>