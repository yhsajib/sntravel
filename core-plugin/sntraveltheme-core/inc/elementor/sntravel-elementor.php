...
class Pxl_Elementor
{
    ...
    {
        add_action('init', [$this, 'sntravel_elementor_init']);

        //load header, footer, megamenu, hidden panels builder templates
        add_action('wp_enqueue_scripts', [$this, 'enqueue_scripts']);
        //add_action( 'elementor/css-file/post/enqueue', [ $this, 'sntravel_after_enqueue_post_css' ] );
        //add_action( 'elementor/css-file/post/parse', [ $this, 'sntravel_add_all_post_css' ] );
        add_action('elementor/element/parse_css',[$this,'sntravel_add_post_css'], 10, 2);
        ...
        add_action('elementor/editor/before_enqueue_scripts', function () {
            wp_enqueue_style('sntravel-editor-css', SNTRAVEL_URL . 'assets/css/elementor-editor.css', array(), '1.0.0');
        ...
        add_action('elementor/editor/after_enqueue_scripts', function () {
            $awesome_pro_support = apply_filters('sntravel_support_awesome_pro', true);
            if ($awesome_pro_support){
                wp_enqueue_style('font-awesome-pro', SNTRAVEL_URL . 'assets/libs/font-awesome-pro/css/all.min.css', [], '6.0.0-pro');
            }
        });
        ...
        if ($core_main_support){
            add_action('elementor/editor/v2/scripts/enqueue/after', function () {
                wp_enqueue_script('sntravel-core-base', SNTRAVEL_URL . 'assets/js/base.js', [ 'jquery' ], '1.0.0', true);
            });
        }
    ...
    public function sntravel_elementor_init(){
        if( is_admin() ){
            $e_font_icon_svg_opt = get_option( 'elementor_experiment-e_font_icon_svg', 'default' );
            $e_font_icon_svg_filter = apply_filters( 'sntravel-e-font-icon-svg-force-inactive', false);
    ...
    public function enqueue_scripts(){
        // not active sntravel theme
        $theme_builder_layout_ids = [];
        $bd_layout_ids = apply_filters('sntravel_theme_builder_layout_ids', $theme_builder_layout_ids);
    ...
    public function sntravel_after_enqueue_post_css($css_file){ 
        $pid = $css_file->get_post_id(); 
        $post_type = get_post_type($pid);
        if( $post_type == 'sntravel-template' || $post_type == 'sntravel-slider'){
            $css_file = Post_CSS::create( $pid );
            $css_file->enqueue();
        } 
    ...
    public function sntravel_add_all_post_css($post_css){
    ...
    public function sntravel_add_post_css($post_css, $element){
        $wg_datas = apply_filters('sntravel_wg_get_source_id_builder', []);
    ...
    {
        $post_type_df = ['sntravel-template'];
        $post_type_builder = apply_filters('sntravel_theme_builder_post_types', $post_type_df);
    ...
    public function set_el_defaults()
    {
        ...
        if (!$cpt_support) {
            $cpt_support = ['page', 'post', 'portfolio', 'sntravel-template']; //create array of our default supported post types
            $cpt_support = apply_filters('sntravel_add_cpt_support', $cpt_support);
    ...
    public function register_elementor_categories($elements_manager)
    {
        ...
        $categories['sntraveltheme-core'] =
            [
                'title' => pixelart()->plugin_name, // esc_html__( 'Pxlart Core', SNTRAVEL_TEXT_DOMAIN ),  
                'icon' => 'fa fa-plug'
            ];
    ...
    public function elements_registered($el_manager){
        ...
        $el_manager->register_element_type( new \Elementor\SNTRAVEL_Element_Section() );
        $el_manager->register_element_type( new \Elementor\SNTRAVEL_Element_Column() ); 
        $el_manager->register_element_type( new \Elementor\Includes\Elements\SNTRAVEL_Element_Container() ); 
    ...
    public function register_widgets($widgets_manager)
    {
        ...
        require_once(SNTRAVEL_PATH . 'inc/elementor/widgets/abstract-class-widget-base.php');
        require_once(SNTRAVEL_PATH . 'inc/elementor/widgets/sntravel-widget-base.php');
        // Scan element (need add to bottom of this file)
        $folder = apply_filters( 'sntravel-register-widgets-folder', get_template_directory() . '/elements/register/' );
        ...
        $sntravel_widgets = apply_filters('sntravel_register_widgets', []);
        if( !empty($sntravel_widgets)){
            
            foreach ($sntravel_widgets as $widget) {
    ...
    public function init_controls()
    {
        ...
        require_once(__DIR__ . '/sntravel-controls/class-control-layout.php');
   
        $controls_manager->register_control(null, new SntravelTheme_Core_Layout_Control());

        $is_sp_control_icons = apply_filters( 'sntravel_support_e_control_icons', true );
        if( $is_sp_control_icons){
            require_once(__DIR__ . '/sntravel-controls/class-control-icons.php');
            $controls_manager->register_control(null, new SntravelTheme_Core_Icons_Control());
        }

        $is_sp_control_list = apply_filters( 'sntravel_support_e_control_list', true );
        if( $is_sp_control_list){
            require_once(__DIR__ . '/sntravel-controls/class-control-list.php');
            $controls_manager->register_control(null, new SntravelTheme_Core_List_Control());
        }
        // Add Tab
        $controls_manager->add_tab('sntraveltheme_core', pixelart()->plugin_name);
    }
