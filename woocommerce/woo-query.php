...
class Pxl_Woo_Query {
	...
	public function basilico_woocommerce_query($type='recent_product',$post_per_page=-1,$product_ids='',$categories='',$param_args=[]){
	    global $wp_query;
	    ...
	    if(!empty($product_ids)){
	        ...
	        $args = $this->basilico_product_filter_type_args($type,$args);
	...
	public function basilico_product_filter_type_args($type,$args){
        $product_visibility_term_ids = wc_get_product_visibility_term_ids();
	    switch ($type) {
	        case 'best_selling':
	            $args['meta_key']='total_sales';
	...
	public function basilico_woocommerce_query_search($type = 'recent_product', $post_per_page = -1, $param_args = []){
	    global $wp_query;
       ...
       /* if( !empty($param_args['pro_atts']) ){
		...
		if( !empty($param_args['sku']) || !empty($param_args['title']) || !empty($param_args['excerpt']) || !empty($param_args['content']) ){	 
		 	add_filter( 'posts_where', 'basilico_search_where_product_filter', 10, 2 );
		}
