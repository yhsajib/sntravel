...
class Pxl_El_Template_Library
{
    ...
    public function __construct(){
        ...
        add_action( 'elementor/init', [ $this, 'sntravel_egister_template_library_source' ], 15 );
    ...
    function sntravel_egister_template_library_source(){
            ...
            if( file_exists($info_file_path) ){
                ...
                if( !empty( $info_data['library']['types_data']['block']['categories'])){
                    if( !empty( $e_library_data['types_data']['block']['categories'] )){
                        foreach( $info_data['library']['types_data']['block']['categories'] as $sntravel_cat){
                            if( !in_array( $sntravel_cat, $e_library_data['types_data']['block']['categories']) ){
                                array_unshift($e_library_data['types_data']['block']['categories'], $sntravel_cat);
                                array_unshift($e_library_data['categories'], $sntravel_cat);
                            }
                        }
